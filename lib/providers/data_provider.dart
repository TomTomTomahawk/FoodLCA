...
class DataProvider {
  ...
  static Future<List<Map<String, dynamic>>> getRecipeIngredientsList(
      recipeid) async {
    if (db == null) {
      await open();
    }
...
//SELECT EXISTS(SELECT * FROM recipes WHERE draft = TRUE)
  ...
  static Future<List<Map<String, dynamic>>> getRecipeNameFromId(
      recipeid) async {
    if (db == null) {
      await open();
    }
  ...
  static Future<List<Map<String, dynamic>>> getCompareRecipeIngredientsList(
      recipeid, comparerecipeid) async {
    if (db == null) {
      await open();
    }
    //return await db.query('Ingredients'); WHERE draft = TRUE'''
  ...
  static Future updateRecipe(Map<String, dynamic> recipe) async {
    await db
        .update('recipes', recipe, where: 'id = ?', whereArgs: [recipe['id']]);
  }
